import { Code2, Mail, MapPin } from "lucide-react";
import { Link as RouterLink } from "react-router-dom";
import { Link } from "react-scroll";

const serviceLinks = [
  "Web Development",
  "AI Implementation",
  "Business Automation",
  "Mobile Apps",
  "Custom CRMs",
  "Chatbots",
];

const companyLinks = [
  { label: "About", to: "about" },
  { label: "Portfolio", to: "portfolio" },
  { label: "Reviews", to: "reviews" },
  { label: "Process", to: "process" },
  { label: "Contact", to: "contact" },
];

export function Footer() {
  return (
    <footer className="bg-navy-950 border-t border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          {/* Brand */}
          <div className="lg:col-span-1">
            <div className="flex items-center gap-2.5 mb-4">
              <Code2 className="w-8 h-8 text-pink-500" />
              <span className="text-[17px] font-bold text-white tracking-tight">
                DayBreak<span className="text-pink-500 font-bold"> Tech-Innovations</span>
              </span>
            </div>
            <p className="text-sm text-slate-500 leading-relaxed mb-5">
              Practical tech solutions for startups and growing businesses.
              We build the tools that help you move faster and grow smarter.
            </p>
            <div className="flex gap-3">
              <a
                href="https://www.instagram.com/daybreakweb_?igsh=N29xbDNyM3U0ejZp"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-8 h-8 bg-white/5 hover:bg-white/10 rounded-lg transition-all text-slate-400 hover:text-white"
                aria-label="Instagram"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href="mailto:Daybreaktechinnovations@gmail.com"
                className="flex items-center justify-center w-8 h-8 bg-white/5 hover:bg-white/10 rounded-lg transition-all text-slate-400 hover:text-white"
                aria-label="Email"
              >
                <Mail className="w-4 h-4" />
              </a>
            </div>
          </div>

          {/* Services */}
          <div>
            <h4 className="text-sm font-semibold text-white mb-4">Services</h4>
            <ul className="space-y-2.5">
              {serviceLinks.map((s) => (
                <li key={s}>
                  <Link
                    to="services"
                    smooth
                    duration={600}
                    offset={-64}
                    className="text-sm text-slate-500 hover:text-slate-300 transition-colors cursor-pointer"
                  >
                    {s}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Company */}
          <div>
            <h4 className="text-sm font-semibold text-white mb-4">Company</h4>
            <ul className="space-y-2.5">
              {companyLinks.map((link) => (
                <li key={link.label}>
                  <Link
                    to={link.to}
                    smooth
                    duration={600}
                    offset={-64}
                    className="text-sm text-slate-500 hover:text-slate-300 transition-colors cursor-pointer"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
              <li>
                <RouterLink
                  to="/careers"
                  className="text-sm text-slate-500 hover:text-slate-300 transition-colors"
                >
                  Careers
                </RouterLink>
              </li>
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="text-sm font-semibold text-white mb-4">Contact</h4>
            <ul className="space-y-3">
              <li className="flex items-start gap-2.5 text-sm text-slate-500">
                <Mail className="w-4 h-4 text-slate-600 flex-shrink-0 mt-0.5" />
                <a
                  href="mailto:Daybreaktechinnovations@gmail.com"
                  className="hover:text-slate-300 transition-colors break-all"
                >
                  Daybreaktechinnovations@gmail.com
                </a>
              </li>
              <li className="flex items-start gap-2.5 text-sm text-slate-500">
                <MapPin className="w-4 h-4 text-slate-600 flex-shrink-0 mt-0.5" />
                <span>Cape Town, South Africa</span>
              </li>
              <li className="flex items-start gap-2.5 text-sm text-slate-500">
                <MapPin className="w-4 h-4 text-slate-600 flex-shrink-0 mt-0.5" />
                <span>Ho Chi Minh City, Vietnam</span>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="border-t border-white/5 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-slate-600">
            © {new Date().getFullYear()} Daybreak Tech Innovations. All rights reserved.
          </p>
          <p className="text-xs text-slate-600">
            Crafted with precision in Cape Town &amp; Ho Chi Minh City
          </p>
        </div>
      </div>
    </footer>
  );
}
